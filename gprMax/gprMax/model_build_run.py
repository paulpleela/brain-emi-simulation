...
def solve_gpu(currentmodelrun, modelend, G):
    ...
    # Transmission lines - hybrid GPU/CPU bridge.
    # TL states are 1D arrays on CPU; fields are on GPU. We sample line current
    # from GPU H-fields, update TL states on CPU, then inject the TL electric
    # source value back onto GPU fields each timestep.
    if G.transmissionlines:
        kernels_tl_bridge = SourceModule("""

        #define INDEX2D(a, b) (a) * 4 + (b)
        #define INDEX3D(i, j, k) (i) * ($NY_FIELDS) * ($NZ_FIELDS) + (j) * ($NZ_FIELDS) + (k)

        __global__ void sample_tl_currents(int N,
                                           const int* __restrict__ tlcoords,
                                           $REAL dx, $REAL dy, $REAL dz,
                                           const $REAL* __restrict__ Hx,
                                           const $REAL* __restrict__ Hy,
                                           const $REAL* __restrict__ Hz,
                                           $REAL* __restrict__ tlcurrents) {
            int n = blockIdx.x * blockDim.x + threadIdx.x;
            if (n >= N) return;

            int x = tlcoords[INDEX2D(n, 0)];
            int y = tlcoords[INDEX2D(n, 1)];
            int z = tlcoords[INDEX2D(n, 2)];
            int pol = tlcoords[INDEX2D(n, 3)];
            ...
            if (pol == 0) {
                ...
                if (y == 0 || z == 0) {
                    ...
                    tlcurrents[n] = dy * (Hy[INDEX3D(x, y, z - 1)] - Hy[INDEX3D(x, y, z)])
                                  + dz * (Hz[INDEX3D(x, y, z)] - Hz[INDEX3D(x, y - 1, z)]);
                }
            } else if (pol == 1) {
                // Iy
                if (x == 0 || z == 0) {
                    ...
                    tlcurrents[n] = dx * (Hx[INDEX3D(x, y, z)] - Hx[INDEX3D(x, y, z - 1)])
                                  + dz * (Hz[INDEX3D(x - 1, y, z)] - Hz[INDEX3D(x, y, z)]);
                }
            } else {
                // Iz
                if (x == 0 || y == 0) {
                    ...
                    tlcurrents[n] = dx * (Hx[INDEX3D(x, y - 1, z)] - Hx[INDEX3D(x, y, z)])
                                  + dy * (Hy[INDEX3D(x, y, z)] - Hy[INDEX3D(x - 1, y, z)]);
                }
            }
        }

        __global__ void inject_tl_fields(int N,
                                         const int* __restrict__ tlcoords,
                                         const $REAL* __restrict__ tlef,
            ...
            if (pol == 0) {
                Ex[INDEX3D(x, y, z)] = tlef[n];
            } else if (pol == 1) {
                Ey[INDEX3D(x, y, z)] = tlef[n];
            } else {
                Ez[INDEX3D(x, y, z)] = tlef[n];
            }
        }
        """.replace('$REAL', cudafloattype).replace('$NY_FIELDS', str(G.ny + 1)).replace('$NZ_FIELDS', str(G.nz + 1)), options=compiler_opts)

        sample_tl_currents_gpu = kernels_tl_bridge.get_function("sample_tl_currents")
        inject_tl_fields_gpu = kernels_tl_bridge.get_function("inject_tl_fields")

        tlcoords = np.zeros((len(G.transmissionlines), 4), dtype=np.int32)
        for tlidx, tl in enumerate(G.transmissionlines):
            tlcoords[tlidx, 0] = tl.xcoord
            tlcoords[tlidx, 1] = tl.ycoord
            tlcoords[tlidx, 2] = tl.zcoord
        ...
        tlcoords_gpu = gpuarray.to_gpu(tlcoords)
        tlcurrents_gpu = gpuarray.zeros(len(G.transmissionlines), dtype=floattype)
        tlef_host = np.zeros(len(G.transmissionlines), dtype=floattype)
        tlef_gpu = gpuarray.zeros(len(G.transmissionlines), dtype=floattype)
    ...
    if G.transmissionlines and G.messages:
        print('GPU TL hybrid bridge active for {} transmission lines'.format(len(G.transmissionlines)))
    ...
    for iteration in tqdm(range(G.iterations), desc='Running simulation, model ' + str(currentmodelrun) + '/' + str(modelend), ncols=get_terminal_width() - 1, file=sys.stdout, disable=not G.progressbars):
        ...
        # Store TL total voltage/current outputs for this iteration
        ...
        # Update TL current from GPU magnetic fields (hybrid bridge)
        if G.transmissionlines:
            sample_tl_currents_gpu(np.int32(len(G.transmissionlines)),
                                   tlcoords_gpu.gpudata,
                                   floattype(G.dx), floattype(G.dy), floattype(G.dz),
                                   G.Hx_gpu.gpudata, G.Hy_gpu.gpudata, G.Hz_gpu.gpudata,
                                   tlcurrents_gpu.gpudata,
                                   block=(1, 1, 1), grid=(round32(len(G.transmissionlines)), 1, 1))

            tlcurrents = tlcurrents_gpu.get()
            for tlidx, tl in enumerate(G.transmissionlines):
                tl.update_current(iteration, G)
                tl.current[tl.antpos] = tlcurrents[tlidx]
        ...
        # Update TL voltage and inject equivalent electric field into GPU fields
        if G.transmissionlines:
            for tlidx, tl in enumerate(G.transmissionlines):
                tl.update_voltage(iteration, G)
                if tl.polarisation == 'x':
                    tlef_host[tlidx] = -tl.voltage[tl.antpos] / G.dx
                elif tl.polarisation == 'y':
                    tlef_host[tlidx] = -tl.voltage[tl.antpos] / G.dy
                else:
                    tlef_host[tlidx] = -tl.voltage[tl.antpos] / G.dz

            tlef_gpu.set(tlef_host)
            inject_tl_fields_gpu(np.int32(len(G.transmissionlines)),
                                 tlcoords_gpu.gpudata,
                                 tlef_gpu.gpudata,
                                 G.Ex_gpu.gpudata, G.Ey_gpu.gpudata, G.Ez_gpu.gpudata,
                                 block=(1, 1, 1), grid=(round32(len(G.transmissionlines)), 1, 1))
    ...
    if G.transmissionlines and G.messages:
        for tlidx, tl in enumerate(G.transmissionlines[:2], start=1):
            print('GPU TL diagnostic tl{}: max|Vtotal|={:.6g}, max|Itotal|={:.6g}'.format(
                tlidx,
                float(np.max(np.abs(tl.Vtotal))),
                float(np.max(np.abs(tl.Itotal)))
            ))
