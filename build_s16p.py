...
For each scenario:
  ...
  - Reference impedance Z0 = 73 Ohm (matches the dipole #transmission_line)
  - Frequency range kept: 0.5 - 2.0 GHz (strips DC and sub-band noise)
  - Saves one .s16p Touchstone file per scenario
  - Deletes the 16 .out files to save disk space (disabled by default)

Usage:
    python build_s16p.py --scenario 1
    python build_s16p.py --all
    python build_s16p.py --range 1 300
...
def write_s16p(S, freqs, out_path, scenario_id):
    """
    Write S-matrix as Touchstone .s16p (magnitude-angle, GHz).
    ...
    with open(out_path, 'w') as f:
        ...
        f.write("! Generated by build_s16p.py\n")
...
def process_scenario(scenario_id):
    """Full pipeline for one scenario: read -> compute S -> write .s16p."""
...
def main():
    ...
    group.add_argument("--scenario", type=int, metavar="N", help="Process a single scenario number")
    group.add_argument("--all", action="store_true", help="Process all scenarios (1-300)")
    group.add_argument("--range", type=int, nargs=2, metavar=("START", "END"),
                       help="Process scenarios START to END (inclusive)")
    parser.add_argument("--no-delete", action="store_true",
                        help="Keep .out files after extraction (default: keep)")
    args = parser.parse_args()

    global DELETE_OUT
    DELETE_OUT = not args.no_delete and DELETE_OUT
