...
for src_idx in range(n_antennas):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        f.write(f"cell = 0.002  # grid resolution\n")
        ...
        f.write(f"    gp_inner_x = cx + ux * gp_thickness\n")
        f.write(f"    gp_inner_y = cy + uy * gp_thickness\n")
        f.write(f"    feed_x = gp_inner_x + ux * cell\n")
        f.write(f"    feed_y = gp_inner_y + uy * cell\n")
        ...
        f.write(f"    antenna_positions.append((cx, cy, cz, ux, uy, gp_inner_x, gp_inner_y, feed_x, feed_y, tip_x, tip_y, pol))\n")
        f.write(f"#end_python:\n\n")
        ...
        for ant_idx in range(n_antennas):
            ...
            f.write("# GP: PEC cylinder disc, axis from outer face (cx,cy) to inner face (gp_inner)\n")
            f.write("# radius=gp_half_size. Feed is one cell beyond inner face -> outside PEC.\n")
