...
Options:
  ...
  --cpu               Use CPU mode
  --tx-sequential     In GPU mode, run legacy single-job sequential mode
  --tx-concurrency N  In GPU mode, max concurrent scenario jobs
  --tx-cpus N         In GPU mode, CPUs per scenario task (default: 8)
...
Notes:
  - Default GPU mode submits run_simulation_gpu.sh, which launches one SLURM array
    task per scenario; each task runs all 16 TX for that scenario.
  - A dependent CPU post job then builds FD tensors and normalization stats.
...
if [[ -n "$TX_CONCURRENCY" ]]; then
  echo "Scenario concurrency cap: ${TX_CONCURRENCY}"
else
  echo "Scenario concurrency cap: default"
fi
echo "GPU cpus-per-scenario-task: ${TX_CPUS}"
...
submit_gpu_parallel_pipeline() {
  local export_vars
  export_vars="ALL,START_SCENARIO=${START_SCENARIO},END_SCENARIO=${END_SCENARIO},DELETE_IN=${DELETE_IN},DELETE_OUT=${DELETE_OUT},GPU_CPUS_PER_TASK=${TX_CPUS}"
  if [[ -n "${TX_CONCURRENCY}" ]]; then
    export_vars="${export_vars},MAX_CONCURRENT_SCENARIOS=${TX_CONCURRENCY}"
  fi
  mkdir -p logs
  echo "Submitting GPU scenario-parallel SLURM workflow..."
  sbatch --export="${export_vars}" run_simulation_gpu.sh
}
